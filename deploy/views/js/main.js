...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    // OPTIMIZATION: Store results of query so we don't have to keep
    // re-calling querySelectorAll()
    var pizzas = document.querySelectorAll(".randomPizzaContainer");

    // OPTIMIZATION: Calc all widths in one loop, then set all styles in a
    // subsequent loop to avoid layout thrashing.
    var newWidths = [];

    // OPTIMIZATION NOTE: The pizza images are all the same size in the current
    // setup, so could possibly just calc the newWidth for one and reuse that
    // value. However, it's possible for them to have smaller sizes, so leaving
    // that optimization out and keeping the calculation for each.
...
function updatePositions() {
  ...
  // OPTIMIZATION: Store the values so we only have to recalculate layout
  // once instead each time through the loop below. (Also, changed to use
  // documentElement.scrollTop for Firefox, though didn't test performance there)
  ...
  for (var i = 0; i < items.length; i++) {
    // OPTIMIZATION: Use stored value instead of calculating layout for
    // body.scrollTop each time. Use a simple counter for the phase offset(?)
    // to avoid calculating modulo.
    ...
    // OPTIMIZATION: Only move the pizzas we can see, and remove ones we can't
    // from the Render Tree. And use stored values instead of recalculating layout.
    ...
    if (items[i].basicTop < windowHeight &&
      ...
      // OPTIMIZATION: Use "transform: translate3d()", which just composites and
      // forces to a new GPU Render Layer, instead of "left", which requries layout,
      // paint, and composite.
...
// OPTIMIZATION NOTE: Calling directly from the scroll event performed well
// on my Moto X but calling from requestAnimationFrame did not (even with
// complex checks to throttle it). No difference on my laptop so leaving it as is.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 200; i++) {
    ...
    // OPTIMIZATION: Created PNG at proper size so browser doesn't have to scale
    // it under all circumenstances. Also changed width so image is not stretched.
    // Performance gains on initiation of the scroll.
    elem.src = "images/pizza-small.png";
    elem.style.height = "100px";
    elem.style.width = "77px";
    // OPTIMIZATION: Store a value for width so we don't have to calc layout to
    // get it again.
    elem.basicWidth = 77;
    elem.basicLeft = (i % cols) * s;
    // OPTIMIZATION: Setting "left" to 0 for all pizzas since we'll use transforms
    // to position them later.
    elem.style.left = '0';
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    // OPTIMIZATION: Store a custom property for the top value, so we don't have
    // to recalculate style later.
